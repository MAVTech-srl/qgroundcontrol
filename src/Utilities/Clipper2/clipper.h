...
namespace Clipper2Lib {
    ...
    inline Paths64 BooleanOp(ClipType cliptype, FillRule fillrule, const Paths64& subjects, const Paths64& clips) {
        Paths64 result;
        Clipper64 clipper;
    ...
    inline void BooleanOp(ClipType cliptype, FillRule fillrule, const Paths64& subjects, const Paths64& clips,
                        PolyTree64& solution) {
        Paths64 sol_open;
        Clipper64 clipper;
        clipper.AddSubject(subjects);
    ...
    inline PathsD BooleanOp(ClipType cliptype, FillRule fillrule, const PathsD& subjects, const PathsD& clips,
                            int precision = 2) {
        int error_code = 0;
    ...
    inline void BooleanOp(ClipType cliptype, FillRule fillrule, const PathsD& subjects, const PathsD& clips,
    ...
    inline Paths64 InflatePaths(const Paths64& paths, double delta, JoinType jt, EndType et, double miter_limit = 2.0,
    ...
    inline PathsD InflatePaths(const PathsD& paths, double delta, JoinType jt, EndType et, double miter_limit = 2.0,
    ...
    inline Path64 TranslatePath(const Path64& path, int64_t dx, int64_t dy) { return TranslatePath<int64_t>(path, dx, dy); }

    inline PathD TranslatePath(const PathD& path, double dx, double dy) { return TranslatePath<double>(path, dx, dy); }
    ...
    inline PathsD RectClip(const RectD& rect, const PathsD& paths, int precision = 2) {
        ...
        return ScalePaths<double, int64_t>(rc.Execute(pp), 1 / scale, error_code);
    }
    ...
    inline Paths64 RectClipLines(const Rect64& rect, const Path64& line) { return RectClipLines(rect, Paths64{line}); }
    ...
    inline void PolyPathToPaths64(const PolyPath64& polypath, Paths64& paths) {
        ...
        for (const auto& child : polypath) PolyPathToPaths64(*child, paths);
    }

    inline void PolyPathToPathsD(const PolyPathD& polypath, PathsD& paths) {
        paths.emplace_back(polypath.Polygon());
        for (const auto& child : polypath) PolyPathToPathsD(*child, paths);
    }
    ...
    inline bool PolyPath64ContainsChildren(const PolyPath64& pp) {
        for (const auto& child : pp) {
            ...
            if (child->Count() > 0 && !PolyPath64ContainsChildren(*child)) return false;
        }
        return true;
    }

    static void OutlinePolyPath(std::ostream& os, size_t idx, bool isHole, size_t count, const std::string& preamble) {
    ...
    static void OutlinePolyPath64(std::ostream& os, const PolyPath64& pp, size_t idx, std::string preamble) {
        OutlinePolyPath(os, idx, pp.IsHole(), pp.Count(), preamble);
        for (size_t i = 0; i < pp.Count(); ++i)
            if (pp.Child(i)->Count()) details::OutlinePolyPath64(os, *pp.Child(i), i, preamble + "  ");
    }

    static void OutlinePolyPathD(std::ostream& os, const PolyPathD& pp, size_t idx, std::string preamble) {
        OutlinePolyPath(os, idx, pp.IsHole(), pp.Count(), preamble);
        for (size_t i = 0; i < pp.Count(); ++i)
            if (pp.Child(i)->Count()) details::OutlinePolyPathD(os, *pp.Child(i), i, preamble + "  ");
    }

    template <typename T, typename U>
    inline constexpr void MakePathGeneric(const T an_array, size_t array_size, std::vector<U>& result) {
    ...
    inline std::ostream& operator<<(std::ostream& os, const PolyTree64& pp) {
        ...
        for (size_t i = 0; i < pp.Count(); ++i)
            if (pp.Child(i)->Count()) details::OutlinePolyPath64(os, *pp.Child(i), i, "  ");
    ...
    inline std::ostream& operator<<(std::ostream& os, const PolyTreeD& pp) {
        ...
        os << std::endl << "Polytree with " << pp.Count() << plural << std::endl;
        for (size_t i = 0; i < pp.Count(); ++i)
            if (pp.Child(i)->Count()) details::OutlinePolyPathD(os, *pp.Child(i), i, "  ");
        os << std::endl << std::endl;
    ...
    inline Paths64 PolyTreeToPaths64(const PolyTree64& polytree) {
        Paths64 result;
        for (const auto& child : polytree) details::PolyPathToPaths64(*child, result);
        return result;
    }

    inline PathsD PolyTreeToPathsD(const PolyTreeD& polytree) {
        PathsD result;
        for (const auto& child : polytree) details::PolyPathToPathsD(*child, result);
        return result;
    }

    inline bool CheckPolytreeFullyContainsChildren(const PolyTree64& polytree) {
        for (const auto& child : polytree)
            if (child->Count() > 0 && !details::PolyPath64ContainsChildren(*child)) return false;
        return true;
    }

    template <typename T,
            typename std::enable_if<std::is_integral<T>::value && !std::is_same<char, T>::value, bool>::type = true>
    inline Path64 MakePath(const std::vector<T>& list) {
        const auto size = list.size() - list.size() % 2;
        if (list.size() != size) DoError(non_pair_error_i);  // non-fatal without exception handling
        Path64 result;
    ...
    template <typename T,
            typename std::enable_if<std::is_arithmetic<T>::value && !std::is_same<char, T>::value, bool>::type = true>
    ...
    inline Path64 MakePathZ(const T2 (&list)[N]) {
        ...
        std::size_t size = N / 3;
        Path64 result(size);
        for (size_t i = 0; i < size; ++i) result[i] = Point64(list[i * 3], list[i * 3 + 1], list[i * 3 + 2]);
        return result;
    }

    template <typename T2, std::size_t N>
    inline PathD MakePathZD(const T2 (&list)[N]) {
        static_assert(N % 3 == 0, "MakePathZD requires values in multiples of 3");
        std::size_t size = N / 3;
        PathD result(size);
        if constexpr (std::numeric_limits<T2>::is_integer)
            for (size_t i = 0; i < size; ++i) result[i] = PointD(list[i * 3], list[i * 3 + 1], list[i * 3 + 2]);
        else
            for (size_t i = 0; i < size; ++i)
                result[i] = PointD(list[i * 3], list[i * 3 + 1], static_cast<int64_t>(list[i * 3 + 2]));
        return result;
    }
    #endif
    ...
    inline Path64 TrimCollinear(const Path64& p, bool is_open_path = false) {
        ...
        if (!is_open_path) {
            while (srcIt != stop && IsCollinear(*stop, *srcIt, *(srcIt + 1))) ++srcIt;
            while (srcIt != stop && IsCollinear(*(stop - 1), *stop, *srcIt)) --stop;
        ...
        if (is_open_path)
            ...
            while (dst.size() > 2 && IsCollinear(dst[dst.size() - 1], dst[dst.size() - 2], dst[0])) dst.pop_back();
    ...
    inline double Length(const Path<T>& path, bool is_closed_path = false) {
        ...
        for (; it != stop; ++it) result += Distance(*it, *(it + 1));
        if (is_closed_path) result += Distance(*stop, *path.cbegin());
        return result;
    }

    template <typename T>
    inline bool NearCollinear(const Point<T>& pt1, const Point<T>& pt2, const Point<T>& pt3,
    ...
    inline Path<T> Ellipse(const Rect<T>& rect, size_t steps = 0) {
        return Ellipse(rect.MidPoint(), static_cast<double>(rect.Width()) * 0.5, static_cast<double>(rect.Height()) * 0.5,
                    steps);
    }

    template <typename T>
    inline Path<T> Ellipse(const Point<T>& center, double radiusX, double radiusY = 0, size_t steps = 0) {
        if (radiusX <= 0) return Path<T>();
        if (radiusY <= 0) radiusY = radiusX;
        if (steps <= 2) steps = static_cast<size_t>(PI * sqrt((radiusX + radiusY) / 2));
    ...
    template <typename T>
    inline Path<T> SimplifyPath(const Path<T>& path, double epsilon, bool isClosedPath = true) {
        ...
        for (size_t i = 1; i < high; ++i) distSqr[i] = PerpendicDistFromLineSqrd(path[i], path[i - 1], path[i + 1]);
    ...
    template <typename T>
    inline Paths<T> SimplifyPaths(const Paths<T>& paths, double epsilon, bool isClosedPath = true) {
        Paths<T> result;
        result.reserve(paths.size());
        for (const auto& path : paths) result.emplace_back(std::move(SimplifyPath(path, epsilon, isClosedPath)));
        return result;
    }
    ...
    template <typename T>
    inline void RDP(const Path<T> path, std::size_t begin, std::size_t end, double epsSqrd, std::vector<bool>& flags) {
    ...
    inline Paths<T> RamerDouglasPeucker(const Paths<T>& paths, double epsilon) {
        ...
        result.reserve(paths.size());
        std::transform(paths.begin(), paths.end(), back_inserter(result),
                    [epsilon](const auto& path) { return RamerDouglasPeucker<T>(path, epsilon); });
        return result;
    }
