...
#define RTREE_DONT_USE_MEMPOOLS  // This version does not contain a fixed memory allocator, fill in lines with EXAMPLE
                                 // to implement one.
...
/// ELEMTYPEREAL Type of element that allows fractional and large values such as float or double, for use in volume
/// calcs
///
/// NOTES: Inserting and removing data requires the knowledge of its constant Minimal Bounding Rectangle.
///        This version uses new/delete for nodes, I recommend using a fixed size allocator for efficiency.
///        Instead of using a callback function for returned results, I recommend and efficient pre-sized, grow-only
///        memory array similar to MFC CArray or STL Vector for returning search query result.
///
template <class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL = ELEMTYPE, int TMAXNODES = 8,
...
class RTree {
   ...
   public:
    ...
    /// \param a_searchResult Search result array.  Caller should set grow size. Function will reset, not append to
    /// array.
    ...
    /// \param a_min Min of search bounding rect
    /// \param a_max Max of search bounding rect
    /// \param a_resultCallback Callback function to return result.  The Callback takes both the resulting data point
    /// and the calculated square distance. Callback should return 'true' to continue searching
    /// \return Returns the number of entries found
...
class RTFileStream {
   ...
   public:
    ...
    bool OpenWrite(const char* a_fileName) { return this->Open(a_fileName, "wb"); }
...
RTREE_QUAL::RTree() {
    ...
    if (NUMDIMS < 21) {
        ...
        m_unitSphereVolume =
            (ELEMTYPEREAL)(1.0 / std::sqrt(NUMDIMS * M_PI) * std::pow(2 * M_PI * M_E / NUMDIMS, NUMDIMS / 2.0));
    }
}
...
RTREE_TEMPLATE
void RTREE_QUAL::Remove(const DATATYPE& a_dataId) { RemoveRect(NULL, a_dataId, &m_root); }
...
RTREE_TEMPLATE
size_t RTREE_QUAL::NNSearch(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS],
                            std::function<bool(const DATATYPE&, ELEMTYPE)> callback) const {
    ...
    struct QueueItem {
        QueueItem(Branch* branch, ELEMTYPE distance) : branch(branch), distance(distance) {}

        // Sort in the queue with the minimum distance
        // taking priority
        bool operator<(QueueItem const& a) const { return this->distance > a.distance; }
...
bool RTREE_QUAL::Load(RTFileStream& a_stream) {
    ...
    if ((dataFileId == _dataFileId) && (dataSize == _dataSize) && (dataNumDims == _dataNumDims) &&
        (dataElemSize == _dataElemSize) && (dataElemRealSize == _dataElemRealSize) && (dataMaxNodes == _dataMaxNodes) &&
...
void RTREE_QUAL::ChoosePartition(PartitionVars* a_parVars, int a_minFill) {
    ...
    RTREE_ASSERT((a_parVars->m_count[0] >= a_parVars->m_minFill) && (a_parVars->m_count[1] >= a_parVars->m_minFill));
}
